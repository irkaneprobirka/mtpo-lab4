// Самостоятельные тесты: утверждения выполняет Jest.
const { manacher } = require('../src/manacher.js');
const { parseInput, readInput } = require('../src/json-input.js');
const { parseArguments, formatResult } = require('../src/cli.js');
const { withFile, bruteForce } = require('./helpers.cjs');
const { examples, expectedResult } = require('./cases.cjs');
